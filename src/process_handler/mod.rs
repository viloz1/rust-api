...
pub mod process;
use process::{ProcessStatus, Process};
use toml;
use serde_derive::Deserialize;
use crate::communication::protocols::{Request, RequestType, From, none_request};
...
impl ProcessHandler {
    ...
    pub fn new(mailbox: Receiver<Request>, rocket_mailman: Sender<Request>, handler_mailman: Sender<Request>) -> ProcessHandler {
        ProcessHandler{processes: HashMap::new(), mailbox, rocket_mailman, handler_mailman}
    }
        ...
        loop {
            ...
            match mail {
                ...
                Ok(Request {
                    ...
                    let result = self.rocket_mailman.clone().send(
                        Request{
                            from: From::Handler,
                            rtype: RequestType::GetProcesses,
                            id: None,
                    ...
                    match result {
                        Err(e) => println!("ERROR: Could not send back processes to frontend. Cause: {}",e),
                        _ => ()
                    }
                },
                ...
                Ok(Request {
                    ...
                    let result = process.sender.send(
                        Request{
                            from: From::Handler,
                    ...
                    match result {
                        Err(e) => println!("ERROR: Could not tell process {} to restartpull. Cause: {}",id, e),
                        _ => ()
                    }
                },
                ...
                Ok(Request {
                    ...
                    match process.get_status() {
                        ProcessStatus::Off => {
                            ...
                            match result {
                                Err(e) => println!("ERROR: Could not tell process {} to start. Cause: {}",id, e),
                                _ => ()
                            }
                        },
                ...
                Ok(Request {
                    ...
                    match process.get_status() {
                        ProcessStatus::On => {
                            ...
                            match result {
                                Err(e) => println!("ERROR: Could not tell process {} to stop. Cause: {}",id, e),
                                _ => ()
                            }
                        },
                        _ => println!("Error, process is busy")
                ...
                Ok(Request {
                    ...
                    match process.get_status() {
                        ProcessStatus::On => {
                            ...
                            match result {
                                Err(e) => println!("ERROR: Could not tell process {} to restart. Cause: {}",id, e),
                                _ => ()
                            }
                        },
                        _ => println!("Error, process is busy")
    ...
    fn retrieve_processes(&mut self) -> HashMap<usize, Process> {
        ...
        for process in config.process {
            ...
            map.insert(process.id, Process{path: process.path,
